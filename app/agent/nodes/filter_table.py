import yaml

from app.agent.context import DataAgentContext
from app.agent.llm import llm
from app.agent.state import DataAgentState, TableInfoState
from langgraph.runtime import Runtime
from app.prompt.prompt_loader import load_prompt
from langchain_core.prompts import PromptTemplate
from langchain_core.output_parsers import JsonOutputParser
from app.core.log import logger

async def filter_table(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    writer = runtime.stream_writer
    writer({
        "type": "progress",
        "step": "过滤表信息",
        "status": "running"
    })

    try:
        table_infos: list[TableInfoState] = state["table_infos"]
        query = state["query"]

        prompt = PromptTemplate(
            input_variables=["query", "table_infos"],
            template=load_prompt("filter_table_info"),
        )

        output_parser = JsonOutputParser()
        chain = prompt | llm | output_parser

        result = await chain.ainvoke({'query': query,
                                      'table_infos': yaml.safe_dump(table_infos, allow_unicode=True, sort_keys=False)}
                                    )

        # {
        #     "fact_order": {"order_amount", "region_id"},
        #     "dim_region": {"region_id", "region_name"}
        # }
        filtered_table_infos: list[TableInfoState] = []  # 用于去除重复表信息，列表遍历时候不能边遍历边删除
        for table_info in table_infos:
            if table_info["name"] in result:
                #去重，把table_info中的重复字段信息覆盖掉
                table_info["columns"] = [column_info for column_info in table_info["columns"] if column_info["name"] in result[table_info["name"]]]
                filtered_table_infos.append(table_info)

        writer({
            "type": "progress",
            "step": "过滤表信息",
            "status": "success"
        })

        logger.info(f"过滤表信息：{[filtered_table_info['name'] for filtered_table_info in filtered_table_infos]}")
        return {"table_infos": filtered_table_infos}
    except Exception as e:
        logger.error(f"过滤表信息出错：{e}")
        writer({
            "type": "progress",
            "step": "过滤表信息",
            "status": "error"
        })
        raise
